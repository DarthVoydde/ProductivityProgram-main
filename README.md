ProductivityProgram

This set of Python scripts are an effort to create a program that assists my various productivity needs.

Features in the current version:

- Main Launcher - This feature provides buttons to access other launchers. As of now this isn't needed as the Health Launcher hasn't been
  completed yet.
  
- Productivity Launcher - In this version, launching the program from the Productivity Launcher script is ideal as it's one less page to
  go through. The Productivity Launcher gives you two options; notes and timer. Click either one to use the respective features. Note:
  You can have as many notes and timers windows as you want open at the same time.
  
- Notes - This is a stickypad style notes system that can stay always on top of windows that are set to windowed mode. You can type your
  notes in this window that is resizeable and dynamically adjusts the line size to the size of the window. You can copy and paste text to
  and from the note window. You can save your notes as .txt files or load .txt files into an empty note by using the right-click dropdown
  menu.
  
- Timer - This is a simple timer that you can use when you want to time tasks. I use this to measure the time I've been working on my many
  projects. It offers a start, stop, and reset feature.

  This project will be expanded overtime.

  Coming up: The launcher should be able to be minimized to the arrow menu on the taskbar.
  
